...
namespace dkstlzu.Utility
{
    ...
    {
        public static Color HitColor = Color.cyan;
        ...
        public static RaycastHit2D BoxCast( Vector2 origin, Vector2 size, float angle, Vector2 direction, float distance, int mask ) 
        {
            ...
            //Drawing the cast
            Color castColor = hit ? HitColor : FailColor;
            ...
            if(hit) {
                Debug.DrawLine(hit.point, hit.point + hit.normal.normalized * 0.2f, SecondColor);
            }
#endif
...
#if UNITY_EDITOR
            Debug.Log("PhysicsHelper");
            Color castColor = hit ? HitColor : FailColor;
            Debug.DrawLine(origin, origin + direction, castColor);
#endif
            return hit;
        }
    }
